...
function createUmdConfig(module, env, extract) {
    ...
    const banner = BANNER.replace('{{module}}', isPlugin ? module : '').replace('{{name}}', name);

    const postcssPlugins = isDevelopment ? [autoprefixer()] : [autoprefixer(), csso()];
...
function createEsmConfig(module) {
    ...
    const banner = BANNER.replace('{{module}}', isModule ? module : '').replace('{{name}}', 'default');
